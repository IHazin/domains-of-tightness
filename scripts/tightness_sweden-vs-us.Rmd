...
ggsave("../figures/freq-with-which-norm-domains-come-to-mind.jpeg", dpi = 350)
...
level_order <- c("how people may dress", "being helpful and cooperative", "raise one’s children", "considerate behavior in public", "talk about other groups", "coughing, spitting and sneezing")
...
pooled_data_clean %>% 
 mutate(country = factor(country, level = c("USA", "Sweden")),
  ...
  geom_bar(stat = "identity", color = "black", position = "dodge") +
  geom_errorbar(aes(ymin = tight - 2*se_tight, ymax = tight + 2*se_tight), 
                width = .2, 
                position = position_dodge(width = 0.9)) +
...
ggsave("../figures/domain-specific-tightness.jpeg", dpi = 350)
